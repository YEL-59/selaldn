...
export const PlayerProfileCard = ({ name, team, pos, nationality, foot, dob, apps, image }: PlayerProfileCardProps) => {
  return (
    <div className="w-full bg-white rounded-[20px] p-6 shadow-[0_4px_30px_rgba(0,0,0,0.02)] border border-gray-100/50 flex flex-col gap-8">
      ...
      <div className="w-full rounded-[15px] overflow-hidden bg-gradient-to-b from-cyan-400/5 to-cyan-400/20 border border-cyan-100/30">
        ...
        <div className="p-6">
           <h4 className="text-[20px] font-black text-[#2E004B] tracking-tight mb-1">{name}</h4>
           <p className="text-[12px] font-bold text-gray-400 uppercase tracking-widest">{team} • {pos}</p>
        </div>
      </div>
...
const ProfileStat = ({ label, value, flag }: { label: string; value: string; flag?: string }) => (
  <div className="flex flex-col gap-1.5">
    <h5 className="text-[16px] font-black text-[#2E004B] tracking-tight">{label}</h5>
    <div className="flex items-center gap-2">
       {flag && <span className="text-[14px] grayscale opacity-80">{flag}</span>}
       <span className="text-[13px] font-black text-gray-400/80">{value}</span>
    </div>
  </div>
);
