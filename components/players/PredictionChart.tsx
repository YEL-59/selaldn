  ...
  return (
    <div className="w-full bg-[#37003C] rounded-[5px] p-8 h-[380px] shadow-2xl relative overflow-hidden">
      <div className="flex items-center gap-3 mb-8 relative z-10">
         ...
         <h3 className="text-[18px] font-black text-white tracking-tight">The Prediction (Next 3 GWs)</h3>
      </div>
          ...
          <BarChart data={data} margin={{ top: 10, right: 30, left: -20, bottom: 20 }}>
            ...
            <Bar dataKey="pts" radius={[4, 4, 0, 0]} barSize={45}>
               {data.map((entry, index) => (
                 <Cell 
                   key={`cell-${index}`} 
                   fill={index === 1 ? "url(#purpleGradient)" : "url(#cyanGradient)"} 
                 />
               ))}
            </Bar>
            <defs>
      ...
      <div className="flex justify-around absolute bottom-8 left-0 right-0 px-24 pointer-events-none">
         {data.map((d, i) => (
             <div key={i} className="text-center">
                 <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{d.venue}</p>
             </div>
         ))}
      </div>
    </div>
