...
const StrategyCard = ({ title, btnText, description, transfers }: any) => (
   <div className="bg-[#2B003D] rounded-[2.5rem] p-5 flex flex-col gap-10 shadow-[0_20px_60px_-15px_rgba(43,0,61,0.5)] relative overflow-hidden group border border-white/5 h-full">
      <h3 className="font-black text-white text-[20px] tracking-tight leading-none px-1">{title}</h3>
      ...
      <div className="flex flex-col gap-10 flex-1">
         {transfers.map((t: any, i: number) => (
            <div key={i} className="flex flex-col gap-5">
               ...
               <div className="flex justify-between items-center px-2">
                  <span className="text-[13px] font-black text-[#FF7C7C] tracking-tight opacity-90 uppercase">Player Out</span>
                  <span className="text-[13px] font-black text-white tracking-tight opacity-90 uppercase">Player In</span>
               </div>

               {/* Transfer Pill Row */}
               <div className="flex items-center justify-between  relative">
                  {/* Player Out Pill */}
                  <div className="flex-1 bg-cyan-400 rounded-2xl p-2 flex items-center gap-3 border border-white/30 shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:shadow-[0_0_30px_rgba(34,211,238,0.5)] transition-all">
                     ...
                     <div className="overflow-hidden flex-1">
                        <p className="text-[10px] font-black text-[#1e1b4b] leading-tight mb-1 truncate">{t.out.name}</p>
                        <p className="text-[8px] font-bold text-[#1e1b4b]/60 leading-none truncate">{t.out.team}</p>
                     </div>
                  </div>

                  {/* Points Center */}
                  <div className="flex flex-col items-center shrink-0 min-w-[90px] px-1">
                     <p className="text-[24px] font-black text-[#00F5FF] tracking-tighter leading-none mb-1">{t.gain || "+0.0"}</p>
                     <p className="text-[10px] font-black text-white/50 text-center leading-[1.2] uppercase tracking-tighter">Points Gain<br />Next 3 GW</p>
                  </div>
                  ...
                  <div className="flex-1 bg-cyan-400 rounded-2xl p-2 flex items-center gap-3 border border-white/30 shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:shadow-[0_0_30px_rgba(34,211,238,0.5)] transition-all">
                     ...
                     <div className="overflow-hidden flex-1">
                        <p className="text-[13px] font-black text-[#1e1b4b] leading-tight mb-1 truncate">{t.in.name}</p>
                        <p className="text-[10px] font-bold text-[#1e1b4b]/60 leading-none truncate">{t.in.team}</p>
                     </div>
                  </div>
               </div>
            </div>
         ...
         <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mt-auto">
            <p className="text-[13px] font-medium text-white/70 leading-relaxed text-center italic">
               {description}
            </p>
         </div>
      </div>
      ...
      {/* Final Action Button */}
      <button className="w-full bg-linear-to-r from-cyan-400 to-[#00F5FF] text-[#1e1b4b] py-5 rounded-2xl text-[20px] font-black hover:scale-[1.02] active:scale-95 transition-all shadow-[0_10px_30px_-5px_rgba(0,245,255,0.4)]">
         {btnText}
      </button>
   </div>
);
